...
let valor = 0;
// Requisito 10;
const somandoValores = (price) => {
  valor += price;
  const pegandoTagPrice = document.querySelector('.total-price');
  pegandoTagPrice.innerText = valor.toFixed(2);
};
...
export const createProductElement = ({ id, title, thumbnail, price }) => {
  ...
  // Requisito 8, ajuda colegas e monitoria;
  cartButton.addEventListener('click', async () => {
    saveCartID(id); // Dica de monitoria
    const armazenandoFethProduct = await fetchProduct(id);
    const pegandoClasse = document.querySelector('.cart__products');
    pegandoClasse.appendChild(createCartProductElement(armazenandoFethProduct));
  });
